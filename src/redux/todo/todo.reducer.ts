...
import { generateIndexes, updateAndIndexingData } from '../helpers'
import { StandardAction } from '../type'
import TodoAction from './todo.action'
import { TodoPayload, TodoState } from './type'

export const initData: TodoState = {
  isFetching: false,
  error: null,
  todos: {},
  isUpdatingById: {},
...
export default class TodoReducer {
  static getReducer(
    ...
    const todosFromMeta = (Meta?.collections as any)?.[ReduxCollections.TODO]
  ...
  static handleUpdate = (
    ...
    switch (action.type) {
      case TodoAction.TYPES.UPDATE.START:
        return state
          .setIn(['isUpdatingById', action.payload?.todo.id], true)
          .set('error', '')
      ...
      case TodoAction.TYPES.UPDATE.SUCCESS:
        if (action.payload) {
          return updateAndIndexingData<TodoPayload, TodoState>(
            ...
            ReduxModules.TODO,
            state
          ).setIn(['isUpdatingById', action.payload?.todo.id], false)
        }
        return state
